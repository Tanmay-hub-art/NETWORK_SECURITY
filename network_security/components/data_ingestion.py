...
from network_security.entity.artifact_intity import DataIngestionArtifact
...
class DataIngestion :
    ...
    def export_data_to_feature_store(self,dataframe:pd.DataFrame):
        try:
            feature_store_dir_path = self.data_ingestion_config.feature_store_directory
            os.makedirs(feature_store_dir_path, exist_ok=True)
            
            feature_store_file_path = self.data_ingestion_config.artifact_directory
            dataframe.to_csv(feature_store_file_path, index=False, header=True)
            logging.info(f"Exported dataframe to feature store file: {feature_store_file_path}")
            return dataframe
        except Exception as e:
            raise NetworkSecurityException(e,sys)
